...
xser=0.0, yser=0.0,xver=0.0,yver=0.0,xver1=0.0,xver2=76.9;//координати для руху блоків ліхтаря після другого удару
void Draw()
{
	...
	//рух правого середнього елемента
	glPushMatrix();
	glTranslatef(xser,yser,0.0);
	glColor3f(0.0, 0.0, 0.0);//колір малювання
    ...
    glEnd();
	glPopMatrix();
	//рух правого верхнього сегмента
	glPushMatrix();
	glTranslatef(xver,yver,0.0);
...
y=-160+200*cos(angle4*(3.14/180));//траєкторія руху_4
   ...
   angle=-90.0;//повторення руху катапульти
	...
	//рух правого середнього блоку після другого удару кульки
	if (xser<4)
	{
	yser=-0.08*pow(xser,2);
	xser+=0.0000003;
	}

	if(xser<20)
	{
		yser=(-0.08*pow(xser,2))*0.475;
	xser+=0.003;
	}
	if(xser>3)
	{
    ...
    yser=(-0.08*pow(xser,2))*0.475;
	xser=106.1;
	}// кінець руху правого середнього блоку
	
	//рух правого верхнього блока після другого удару блоку
	yver=0;
		xver+=1;
	
	if(xver>15)
	{
	yver=-0.08*pow(xver1,2);
	xver1+=1;
	}
	
	if(xver1>77.695)
	{
    yver=-0.08*pow(xver2,2);
	xver2=77.695;
	xver=92;
	}//кінець руху правого верхнього блока
